...
class Droid extends Intcode {
  constructor(input) {
    ...
    // fill map
    let mapRow = new Array(50).fill(" ");
    for (let i = 0; i < 50; i++) {
      this.map.push([...mapRow]);
    };
    
    // mark starting point on map
    this.map[this.droidStart.y][this.droidStart.x] = "S";
  }

  renderMap() {

    this.map.forEach(line => {
      fs.appendFileSync("day15_output.txt", line.join("") + "\n" , function(err) {
        if(err) {
  ...
  explore(direction) {
    ...
    // valid direction values: 1, 2, 3, 4
    this.calcOutput(direction);

    // determine new position indicated by direction
    let newPosition = {...this.droidPosition};
    switch (direction) {
      case 1:
        newPosition.y--;
        break;
      ...
      case 4:
        ...
        break;
      default:
        console.log("Incorrect direction input:", direction)
        return;
    }

    // act according to robot output
    let status = this.output[this.output.length - 1];
    ...
    switch (status) {
      ...
      case 1:
        ...
        // set droid position to new position
        this.droidPosition = {...newPosition}
        break;
      case 2:
        this.map[this.droidPosition.y][this.droidPosition.x] = ".";
        this.map[newPosition.y][newPosition.x] = "O";
        this.droidPosition = {...newPosition};
        this.steps++;
        console.log("Oxygen tank at:", this.droidPosition)
        break;
      default:
        console.log("Droid status code indicates error:", status);
        return;
    }
...
function search(droid) {
  ...
  while (!droid.finished) {
    ...
    status = droid.explore(randomDirection) // random number from 1 to 4 (here, 1 is inclusive, 5 exclusive)
